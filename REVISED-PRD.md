...
date: 2026-05-16
app_id: knowledge
app_name: DClaw Knowledge
category: Platform
status: Future
---

# 📘 DClaw Knowledge — Revised PRD v2.3
...
|-------|-------|
| **App ID** | `knowledge` |
| **Name** | DClaw Knowledge |
| **Category** | Platform |
| **Tagline** | Enterprise knowledge graph |
| **Color** | #10B981 |
| **Phase** | Future |
| **Port (Frontend Dev)** | 3048 (TBD — assign before build) |
| **Port (Backend Dev)** | 18118 (TBD — assign before build) |
...
|---|---------|-------------|--------------|---------------------|
| P0.1 | **AI Knowledge Copilot** | Explore and query organizational knowledge via natural language. | GraphRAG + LLM synthesis + entity disambiguation | Answer complex questions spanning 5+ data sources |
| P0.2 | **Entity Extraction** | Auto-extract people, projects, concepts, and relationships from documents. | NER + coreference resolution + relation extraction | Extract 30+ entity types; link mentions; confidence scores |
| P0.3 | **Knowledge Graph Builder** | Visual graph of organizational knowledge with exploration tools. | AI layout optimization + cluster detection | Interactive graph; filter by entity type; shortest-path query |
| P0.4 | **Document Ingestion** | Import and analyze documents from 20+ sources. | AI document-classification + section extraction | Support PDF, HTML, Markdown, Confluence; auto-classify |
...
| # | Feature | Description | AI Component | Acceptance Criteria |
|---|---------|-------------|--------------|---------------------|
| P1.1 | **Expert Finder** | Find subject-matter experts based on content contributions and projects. | AI expertise-scoring + availability prediction | Search by topic; rank by contribution; check availability |
| P1.2 | **Q&A System** | Ask questions and get answers synthesized from multiple sources. | Multi-hop reasoning + source attribution | Cite 3+ sources per answer; confidence score; follow-up questions |
| P1.3 | **Trend Analysis** | Identify emerging topics and trends across organizational content. | AI topic-modeling + temporal-trend detection | Detect 10+ trends monthly; visualize trajectory; alert stakeholders |
| P1.4 | **Integration Hub** | Connect to Slack, Jira, Confluence, SharePoint, and CRM. | AI connector-health monitoring + sync-error detection | 20+ connectors; bi-directional sync; webhook triggers |
...
| # | Feature | Description | AI Component | Acceptance Criteria |
|---|---------|-------------|--------------|---------------------|
| P2.1 | **Semantic Search** | Search across all content using meaning, not just keywords. | Embedding-based retrieval + hybrid keyword | Search 1M documents in <1s; semantic similarity >0.85 |
| P2.2 | **Automated Taxonomy** | Generate and maintain organizational taxonomies. | AI taxonomy-generation + concept-drift detection | Auto-generate 3-level taxonomy; suggest new terms monthly |
| P2.3 | **Privacy Controls** | Redact PII and enforce data residency policies. | AI PII detection + auto-redaction + policy enforcement | Detect 20+ PII types; redact with audit trail; GDPR compliant |
| P2.4 | **API & Webhooks** | Query knowledge graph and receive change notifications. | AI query-optimization + caching strategy | GraphQL API; webhook on entity change; rate limit 1000/min |
...
Inspired by Starmind, Guru, Obsidian, Roam Research. Enterprise knowledge is fragmented; AI graph unlocks it.
