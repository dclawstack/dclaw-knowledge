...
### How do I update DClaw Knowledge?

Update the `version` field in the DClawApp CRD. The operator will perform a rolling update.

```bash
kubectl patch dclawapp knowledge --type merge -p '{"spec":{"version":"0.2.0"}}'
```
...
### Can I run DClaw Knowledge without Kubernetes?

Yes, for local development:

```bash
cd dclaw-knowledge/frontend && npm run dev
cd dclaw-knowledge/backend && uvicorn dclaw_knowledge.main:app --reload
```
