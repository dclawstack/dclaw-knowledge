...
# Check events
kubectl get events -n dclaw-knowledge --sort-by='.lastTimestamp'

# Verify database connection
kubectl exec -n dclaw-knowledge deployment/dclaw-knowledge-backend --   python -c "import asyncio; from sqlalchemy import text; ..."
```
...
2. Check the connection string secret:
   ```bash
   kubectl get secret dclaw-knowledge-db-credentials -n dclaw-knowledge
   ```
